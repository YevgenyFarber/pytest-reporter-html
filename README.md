...
- ✅ **Test History** — each test shows a strip of colored dots for its last 10 runs; hover to see the report filename, status, and duration for any past run
- ✅ **Automatic JSON Cleanup** — intermediate per-test JSON files are deleted after the HTML is generated by default; use `--keep-json` to retain them
...
|---|---|---|
| `--report-html` | off | Generate an aggregated HTML report at session end |
| `--keep-json` | off | Keep intermediate per-test JSON files after HTML generation |
...
Reports are written to `logs/test-reports/`:

```
logs/test-reports/
├── TestReport_Latest.html        ← always the most recent run
├── TestReport_All_2026.03.15_…html
├── TestReport_All_2026.03.14_…html
└── test_history.json             ← cross-run history (auto-managed)
```
...
Every time you run with `--report-html`, the plugin appends the result of each test to `test_history.json` (kept in the report directory, never cleaned up). The last **10 runs** per test are stored.

In the HTML report, each test shows a row of colored dots below its name — one dot per recorded run, oldest on the left and current on the right:

- 🟢 green dot — run passed
- 🔴 red dot — run failed
- ⚫ grey dot — run skipped
- The **current** run dot is full opacity; previous runs are dimmed

Hover any dot to see a tooltip:

```
TestReport_All_2026.03.14_11.30.00.123.html · FAILED · 0.45s
```

This lets you spot flaky tests, performance regressions, and recurring failures at a glance without opening previous reports — but if you need to dig in, the filename in the tooltip tells you exactly which report to open.
